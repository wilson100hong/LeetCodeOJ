...
public:
    // descending!
    int findKth(vector<int>& vec, int l, int r, int kth) {
        int pivot = vec[r];
        int k = 0;
        int i=l, j=r-1;
        while (i<=j) {
        ...
        if (kth == left_len) {
            ...
            return findKth(vec, l, r-k-1, kth);
        } else {
            return findKth(vec, r-k, r-1, kth-left_len-1);
        }
    }

    int maxProfit(int k, vector<int>& prices) {
        if (k==0) return 0;
        int p=-1, v, n=prices.size();
        stack<pair<int, int>> vps;  // pair or (v, p)
        vector<int> profits;
        while (1) {
            for (v=p+1; v+1<n && prices[v+1]<=prices[v]; ++v);
            for (p=v; p+1<n && prices[p+1]>=prices[p]; ++p);

            if (v == p) break;  // reach end
        ...
        int best = 0;
        // pick top k profit from profits
        if (profits.size() <= k) {
        ...
        int pivot = findKth(profits, 0, profits.size() - 1, k-1);
        int cnt = 0;
        for (int profit : profits) {
            if (profit > pivot) {
                //cout << profit << endl;
